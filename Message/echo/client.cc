...
Client::echo() {
	...
	// allow the user to type a messsage
	    string message = parse_message();
	    cout << message << endl;
	    stringstream ss;
	    ss << message.length();
	    string len = ss.str();
...
bool
Client::check_send(string line) {
    stringstream ss (line);
    string command;
    char delim = ' ';
    while(getline(ss,command,delim)) {
	if (command == "send")
	    return true;
	else if (command != "\n")
	    return false; 	
...
string
Client::format_user_input(string input){
    return "";
}
